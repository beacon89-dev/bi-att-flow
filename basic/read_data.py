...
def read_data(config, data_type, ref, data=None, data_filter=None, data_set=None):
    if data is None:
        print("CMG: DATA IS NONE")
        # This is a bit of a hack to just use the test data to initialize some things that it appears
        # we need to initialize the Demo() object.
        demo_data_type = 'test' if data_type=='demo' else data_type
        data_path = os.path.join(config.data_dir, "data_{}.json".format(demo_data_type))
